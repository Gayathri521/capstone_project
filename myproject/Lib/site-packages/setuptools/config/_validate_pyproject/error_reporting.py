...
class ValidationError(JsonSchemaValueException):
    ...
    Depending on the level of the verbosity of the ``Logger`` configuration
    the exception message will be only ``summary`` (default) or a combination of
    ``summary`` and ``details`` (when the Logger level is set to :obj:`Logger.DEBUG`).
    """
...
class _ErrorFormatting:
    ...
    def __str__(self) -> str:
        if _logger.getEffectiveLevel() <= Logger.DEBUG and self.details:
